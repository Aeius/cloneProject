...
function tokenCall2(isReload = true) {
...
function moveAlongRole(){
	if(getCookie('jwt')==0 && !window.location.pathname.endsWith('login')) return location.replace('/login');
...
function userErrorHandler(jqXHR){
...
function adminErrorHandler(jqXHR){
	if (jqXHR.status == 401 && jqXHR.responseText == "토큰 기한 만료") {
		tokenCall2();
	}
	else{
