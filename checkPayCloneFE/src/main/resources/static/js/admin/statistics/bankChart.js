 ...
 * 차트 보여주기
 */
function getBankChart(){
	var chart = null;
    var chartData = getBankChartData();
    
    chart = Highcharts.chart('bankChart', {
	  chart: {
	    type: 'area'
	  },
	  title: {
...
function getBankChartData(){
	var chartData = null;

	$.ajax({
        url: baseUrl + '/api/admin/statistics/bankChartData',
        type: 'get',
        data: {
            selectYear: $('#bankChartData_selectYear').val()
        },
        async: false,
        dataType: 'json',
        ...
        success: function (result) {
            ...
            */
			depositData.map((value, index)=>{
				depositDataObj[value.approved_month+'월'] = Number(value.trans_amt_avg);
		   });
		   withdrawData.map((value, index)=>{
				withdrawDataObj[value.approved_month+'월'] = Number(value.trans_amt_avg);
		   });
